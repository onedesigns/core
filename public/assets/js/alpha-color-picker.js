...
( function( $ ) {
	...
	$.fn.alphaColorPicker = function() {
		 ...
		 */
		function update_alpha_value_on_color_input( alpha, $input, $alphaSlider, update_slider ) {
		 ...
		 */
		function update_alpha_value_on_alpha_slider( alpha, $alphaSlider ) {
		...
		return this.each( function() {
			...
			paletteInput = $input.data( 'palette' );
			showOpacity  = $input.data( 'showOpacity' );
			defaultColor = $input.data( 'defaultColor' ) || '';

			// Process the palette.
			if ( typeof paletteInput == 'string' && paletteInput.indexOf( '|' ) !== -1 ) {
				palette = paletteInput.split( '|' );
			} else if ( 'false' == paletteInput || false == paletteInput ) {
				palette = false;
			} else {
				palette = true;
			}
			...
			$container.addClass( 'wp-picker-container-has-alpha' );
			...
			$container.find( '.min-click-zone' ).on( 'click', function() {
				update_alpha_value_on_color_input( 0, $input, $alphaSlider, true );
			});
			$container.find( '.max-click-zone' ).on( 'click', function() {
				update_alpha_value_on_color_input( 100, $input, $alphaSlider, true );
			});
			...
			$input.on( 'input', function() {
				var value = $( this ).val();
				var alpha = get_alpha_value_from_color( value );
			...
			$alphaSlider.slider().on( 'slide', function( event, ui ) {
				...
				update_alpha_value_on_color_input( alpha, $input, $alphaSlider, false );
