...
function enlightenment_sidebars_settings() {
	...
	if( ! isset( $_GET['template'] ) ) {
		...
		add_settings_field(
			'widgets_link_hover_color',  // Unique identifier for the field for this section
			__( 'Widgets Link Hover Color', 'enlightenment' ), // Setting field label
			'enlightenment_color_picker', // Function that renders the settings field
			'enlightenment_theme_options', // Menu slug, used to uniquely identify the page
			'add_dynamic_sidebar', // Settings section. Same as the first argument in the add_settings_section() above
			array(
				'name' => 'sidebars[' . $sidebar . '][widgets_link_hover_color]',
				'value' => $atts['widgets_link_hover_color'],
			)
		);
		add_settings_field(
			'widgets_link_color',  // Unique identifier for the field for this section
		...
		add_settings_field(
			'widgets_link_active_color',  // Unique identifier for the field for this section
			__( 'Widgets Link Active Color', 'enlightenment' ), // Setting field label
			'enlightenment_color_picker', // Function that renders the settings field
			'enlightenment_theme_options', // Menu slug, used to uniquely identify the page
			'add_dynamic_sidebar', // Settings section. Same as the first argument in the add_settings_section() above
			array(
				'name' => 'sidebars[' . $sidebar . '][widgets_link_active_color]',
				'value' => $atts['widgets_link_active_color'],
			)
		);
	}
...
function enlightenment_unlimited_sidebars_form( $post ) {
	...
	enlightenment_checkbox( array(
		...
		'label' => sprintf( __( 'Use global sidebar locations for %1$s', 'enlightenment' ), $post->post_type ),
	) );
	echo '</p>';
...
function enlightenment_unlimited_sidebars_form_save_postdata( $post_id ) {
	...
	foreach ( $input as $location => $sidebar ) {
		if( '' != $location ) {
			...
			if( 'revision' != $post->post_type && ! array_key_exists( $location, $locations[get_post_type( $post_id )] ) )
